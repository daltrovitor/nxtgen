...
export function PassLevelBanner({ user }: PassLevelBannerProps) {
  ...
  return (
    <div className="rounded-2xl bg-card border border-border p-5 sm:p-7 shadow-sm transition-colors">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-5">
        {/* Left Side: Greeting & Status */}
        <div className="space-y-1">
          <p className="text-xs font-heading font-medium text-muted-foreground">
            Olá, {(user?.name || (user as { fullName?: string })?.fullName || "Membro").split(" ")[0]}
          </p>
          <h1 className="text-2xl sm:text-3xl font-extrabold font-heading text-foreground tracking-tight">
            Você está evoluindo!
          </h1>
          <p className="text-xs sm:text-sm text-muted-foreground font-heading">
            Continue assim para desbloquear novas recompensas.
          </p>
        </div>

        {/* Right Side: Level Card with XP Track */}
        <div className="w-full md:w-80 p-4 rounded-xl bg-slate-50 dark:bg-black/60 border border-slate-200 dark:border-white/10 space-y-3 transition-colors">
          <div className="flex items-center justify-between">
            <div>
              <span className="text-[11px] font-heading text-muted-foreground uppercase tracking-wider block">
                Seu nível atual
              </span>
            ...
            <div className="w-9 h-9 rounded-lg bg-[#8B24F0] flex items-center justify-center font-heading font-extrabold text-sm text-white shadow-sm">
              {level}
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs font-heading text-muted-foreground">
              ...
              <span className="text-purple-600 dark:text-purple-400 font-semibold">{percent}%</span>
            </div>
